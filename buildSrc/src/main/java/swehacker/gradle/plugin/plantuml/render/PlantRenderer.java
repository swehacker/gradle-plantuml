...
public class PlantRenderer {
    ...
    public PlantRenderer(String packageName, Set<Class<?>> types, Set<Relation> relations) {
        this.types = types;
    ...
    private void sortRelations(ArrayList<Relation> relations) {
        relations.sort((o1, o2) -> o1.getClass().equals(o2.getClass()) ? o1.getFromType().getName().compareTo(o1.getFromType().getName()) : o1.getClass().getName().compareTo(o2.getClass().getName()));
    }
    ...
    private void addRelation(StringBuilder sb, Relation relation) {
        ...
        if (!relation.getToType().startsWith(packageName)) return;
        sb.append(relation.generate()).append("\n");
    }

    private boolean isToTypeInAggregations(Relation relation) {
        Class<?> toType = ClassHelper.loadClass(relation.getToType(), this.getClass().getClassLoader());
    ...
    private void addClass(StringBuilder sb, Class<?> aClass) {
        if (aClass.getCanonicalName() != null && !aClass.getCanonicalName().contains(packageName)) return;
    ...
    private void addClassTypeParams(StringBuilder sb, Class<?> aClass) {
        ...
        for (TypeVariable t : aClass.getTypeParameters()) {
            ...
            String jointBounds = ClassHelper.getSimpleName(Strings.join(Arrays.stream(bounds).map(bound -> bound.getTypeName()).collect(Collectors.toList()), "&"));
    ...
    static class MethodPrinter implements MemberPrinter {
        ...
        public void addMember(Member m, List<String> plantMembers) {
            ...
            String returnType = (m instanceof Method) ? " : " + ClassHelper.getSimpleName(((Method) m).getReturnType().getName()) : "";
        ...
        private String buildParams(Member m) {
            ...
            Type[] paramClasses = m instanceof Method ? ((Method) m).getGenericParameterTypes() : ((Constructor<?>) m).getGenericParameterTypes();
