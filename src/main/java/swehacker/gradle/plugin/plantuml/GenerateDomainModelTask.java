...
public class GenerateDomainModelTask extends DefaultTask {
    ...
    @TaskAction
    void generateDomain() throws IOException { //NOSONAR
        List<URL> classpathUrls = new ArrayList<>();
        for (File path : classpath) {
            classpathUrls.add(path.toURL());
        }

        classLoader = new URLClassLoader(classpathUrls.toArray(new URL[0]));
        File buildDir = new File(getProject().getBuildDir(), "classes/java/main");
        this.classLoader = new URLClassLoader(new URL[]{buildDir.toURL()});
        String parsedCode = parse(packageStructure, classLoader);
        File file = new File(outputFile);
        file.getParentFile().mkdirs();
        Files.write(file.toPath(), parsedCode.getBytes());
    }
